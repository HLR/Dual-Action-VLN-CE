...
class CLIPEncoder(nn.Module):
    ...
    def __init__(
        ...
        # sys.path.append("/root/mount/cevln/ov-seg")
        ...
        # cfg.merge_from_file("/root/mount/cevln/ov-seg/configs/ovseg_swinB_vitL_demo.yaml")
        # self.demo = VisualizationDemo(cfg)
        # self.class_names = ["Floor", "Stairs", "Door"]

    def forward(self, observations, obs_land=None, seg_mask=None):
        r"""Sends RGB observation through the TorchVision ResNet50 pre-trained
        on ImageNet. Sends through fully connected layer, activates, and
        returns final embedding.
        """
        if obs_land:
            ...
            for id, ob in enumerate(obs_land):
                # ob_tokens = clip.tokenize(ob).cuda()
                # text_features = self.model.encode_text(ob_tokens).cuda()
                text_features = ob
                image_features = output[id*12:id*12+12]
                image_features = image_features / image_features.norm(dim=-1, keepdim=True)
                text_features = text_features / text_features.norm(dim=-1, keepdim=True)
                clip_score = torch.matmul(image_features, text_features.T).max(dim=1)[0]
                sim_score.append(clip_score)
        
            return output.float(), torch.vstack(sim_score)
        elif seg_mask is not None:
            rgb_observations = observations["rgb"].permute(0, 3, 1, 2)
            rgb_observations = self.rgb_transform(rgb_observations*seg_mask.unsqueeze(1).float())
            """
            seg_mask = []
            for id, each_img in enumerate(rgb_observations):
                img = each_img.cpu().numpy()
                predictions, visualized_output = self.demo.run_on_image(img, self.class_names)
                seg_mask.append(visualized_output)
                # visualized_output.save("/root/mount/cevln/pred.png")
            rgb_observations = observations["rgb"].permute(0, 3, 1, 2)
            stacked_seg_mask = torch.stack(seg_mask,dim=0)
            rgb_observations = self.rgb_transform(rgb_observations*stacked_seg_mask.unsqueeze(1).float())
            """
            #rgb_observations = self.rgb_transform(rgb_observations*seg_mask.unsqueeze(1).float())
            #rgb_observations = self.rgb_transform(rgb_observations*torch.ones_like(rgb_observations))
            #rgb_observations = rgb_observations*seg_mask.unsqueeze(1)
            output = self.model.encode_image(rgb_observations.contiguous())
            return output.float()
