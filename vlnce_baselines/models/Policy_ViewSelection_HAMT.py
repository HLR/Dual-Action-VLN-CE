...
import math
from vlnce_baselines.models.decoders.med import BertConfig, BertModel, BertLMHeadModel
...
class HAMT(Net):
    def __init__(
        ...
        med_config = "/root/mount/dcvln-joslin/vlnce_baselines/models/decoders/med_config.json"
        decoder_config = BertConfig.from_json_file(med_config)        
        self.text_decoder = BertLMHeadModel(config=decoder_config) 
        self.prompt = "low actions: "
    ...
    def forward(self, mode=None, 
            ...
            h_t=None, language_attention=None, targets=None, states=None, obs_land=None, semantic_mask=None):
        ...
        if mode == 'language':
            ...
            semantic_batch = torch.zeros_like(observations['semantic']).repeat(NUM_IMGS, 1, 1)
            ...
            for i, (k, v) in enumerate(observations.items()):
                if 'depth' in k:  # You might need to double check the keys order
                    for bi in range(v.size(0)):
                        ...
                        if semantic_mask:
                            semantic_batch[ra_count + bi*NUM_IMGS] = observations[k.replace('depth','semantic')][bi]
            ...
            if obs_land:
                clip_embedding, clip_score = self.clip_encoder({'rgb':rgb_batch}, obs_land)
                clip_score_index = torch.sort(clip_score, dim=1, descending=True)[1]
                # clip_mask = torch.ones(batch_size, 12).cuda()
                # clip_mask[clip_score_index[:,5:]] = 0
                # clip_mask = clip_mask.view(batch_size*12)
                # video_embedding[clip_score_index[:,5:]] *= 0
                #clip_embedding = clip_embedding.view(batch_size, 12, -1)
            elif semantic_mask:
                clip_embedding1 = self.clip_encoder({'rgb':rgb_batch}, seg_mask = semantic_batch)
            ...
            waypoint_heatmap_logits = waypoint_predictor(
                clip_embedding, depth_embedding)
            ...
            if obs_land:
                topk = 8
                batch_x_norm = batch_x_norm.view(batch_size, NUM_IMGS, 10, NUM_CLASSES)
                new_batch_x_norm = []
                for i in range(batch_size):
                    new_batch_x_norm.append(batch_x_norm[i][clip_score_index[i,:topk]])
                batch_x_norm = torch.stack(new_batch_x_norm,dim=0).view(batch_size, topk*10, NUM_CLASSES)
            ...
            # putting filtering results back
            if obs_land:
                batch_output_map = batch_output_map.view(batch_size, topk, 10, NUM_CLASSES)
                new_batch_output_map = torch.zeros(batch_size, NUM_IMGS, 10, NUM_CLASSES).cuda()
                for i in range(batch_size):
                    new_batch_output_map[i][clip_score_index[i,:topk]] = batch_output_map[i]
                batch_output_map = new_batch_output_map.view(batch_size, NUM_IMGS*10, NUM_CLASSES)
            ...
            for j in range(batch_size):
                ...
                # output = []
                # #images = rgb_batch[np.unique(img_idxes)].cpu().numpy()
                # images = rgb_batch[np.arange(0,12)].cpu().numpy()
            
                # img_list = []
                # for i in range(images.shape[0]):
                # #     img_list.append(images[i])
                # # concate_img = np.concatenate(img_list, 1)
                #     plt.figure(figsize = (100,50))
                #     plt.imshow(images[i])
                #     plt.savefig("/root/mount/cevln/image_examples/test%s.png" % str(i))
        ...
        elif mode == 'low_action':
            prompt_targets = []
            for t in targets:
                prompt_targets.append(self.prompt+t)
            text = self.tokenizer(prompt_targets, padding='longest', truncation=True, max_length=30, return_tensors="pt").to(states.device)
            ...
            decoder_output = self.text_decoder(text.input_ids, 
                                        ...
                                        encoder_hidden_states = states.unsqueeze(1),
                                        encoder_attention_mask = txt_masks[:,0].unsqueeze(1),                  
                                        labels = decoder_targets,
                                        return_dict = True,   
                                        ) 
            return decoder_output.loss 

        elif mode == "low_action_infer":
            num_beams = 3
            model_kwargs = {"encoder_hidden_states": states.unsqueeze(1), "encoder_attention_mask":txt_masks[:,0].unsqueeze(1)}
            
            prompt = [self.prompt] * states.size(0)
            input_ids = self.tokenizer(prompt, return_tensors="pt").input_ids.to(states.device) 
            input_ids[:,0] = self.tokenizer.bos_token_id
            input_ids = input_ids[:, :-1] 
            #bos_ids = torch.full((states.size(0),1),fill_value=self.tokenizer.bos_token_id,device=states.device)
