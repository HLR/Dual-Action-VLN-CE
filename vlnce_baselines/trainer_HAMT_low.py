...
with open("/root/mount/cevln/landmarks/new/train.json") as f_land:
    landmarks = json.load(f_land)
...
class RLTrainer(BaseVLNCETrainer):
    ...
    def _set_config(self):
        ...
        for sensor_type in ["RGB", "DEPTH", "SEMANTIC"]:
    ...
    def _initialize_policy(
        ...
        from vlnce_baselines.waypoint_pred.TRM_net_tmp import BinaryDistPredictor_TRM
        self.waypoint_predictor = BinaryDistPredictor_TRM(device=self.device)
        self.waypoint_predictor.load_state_dict(
            torch.load('/root/mount/cevln/waypoint-predictor/checkpoints/wp-train3/snap/check_latest1', map_location = torch.device('cpu'))['predictor']['state_dict']
        )

        # /root/mount/dcvln-joslin/data/pretrained/wp_pred/waypoint_predictor
        # /root/mount/cevln/waypoint-predictor/checkpoints/wp-train3/snap/check_latest1
    ...
    def compute_low(self, angle, distance):
        ...
        if angle >=0:
            ...
            turn_words = ["left"] * len(turn_actions)
    
        else:
            turn_actions = [right_action] * int(-angle // turn_unit)
            turn_words = ["right"] * len(turn_actions)
        
        ksteps = int(distance//0.25)
        low_actions = turn_words + ['forward']* ksteps
        return low_actions
        
    def landmark_search(self, obs):
        trajectory_ids, instr_list = list(zip(*obs))
        ...
        for i in range(len(trajectory_ids)):
            for j in range(4):
                tmp_id = str(trajectory_ids[i])+"_"+str(j)
                if landmarks[tmp_id]['instr'] == instr_list[i]:
                    tmp_land = landmarks[tmp_id]['land']
                    new_tmp_land = ["a photo of "+i for i in tmp_land]
                    final_landmarks.append(new_tmp_land)
                    bad_case.append(tmp_id)
                    ob_tokens = clip.tokenize(new_tmp_land).cuda()
                    with torch.no_grad():
                        text_features = self.policy.net.module.clip_encoder.model.encode_text(ob_tokens)
                        #text_features = self.policy.net.clip_encoder.model.encode_text(ob_tokens)
                    land_embeds.append(text_features)
                    break
        if final_landmarks == None or len(final_landmarks) != len(obs):
            logger.info(f"wrong cases trj: {trajectory_ids}. wrong cases: {instr_list}.")
        #print(bad_case)
    
        return final_landmarks, land_embeds
    ...
    def _train_iter(self, train_ml=None, sample_ratio=None, train_rl=True):
        ...
        semantic_keys = ['semantic', 'semantic_30', 'semantic_60', 'semantic_90', 'semantic_120', 'semantic_150', 'semantic_180', 'semantic_210', 'semantic_240', 'semantic_270', 'semantic_300', 'semantic_330']
        open_vocab = ["floor", "stairs", "door"]
        for i in range(self.envs.num_envs):
            open_list = []
            instance_id_to_name = self.envs.call_at(i, "get_instance_id", {})
            for key, value in instance_id_to_name.items():
                if value in open_vocab:
                    open_list.append(key)
             
            for s_k in semantic_keys:
                semantic_obs = observations[i][s_k]
                for open_v in open_list:
                    semantic_obs = np.where(semantic_obs==open_v, -1, semantic_obs)
                semantic_obs = np.where(semantic_obs==-1, 1, 0)
                observations[i][s_k] = semantic_obs
            

        instr_max_len = self.config.IL.max_text_len # r2r 80, rxr 200
        instr_pad_id = 1 if self.config.MODEL.task_type == 'rxr' else 0
        #obs_land, obs_embds = self.landmark_search([(ob["instruction"]['trajectory_id'], ob["instruction"]['text']) for ob in observations])
        obs_land, obs_embds = None, None
        ...
        for stepk in range(self.max_len):
            ...
            # if obs_land and len(obs_land) != batch['rgb'].shape[0]:
            #     obs_embds = None
            # if ratio > 0.5:
            #     obs_embds = None
            ...
            for j in range(self.envs.num_envs):
                if cpu_a_t[j].item()==ob_cand_lens[j]-1 or stepk==self.max_len-1:
                    ...
                    each_low = self.compute_low(t_angle, t_distance)
                    low_actions.append(" ".join(each_low)+".")
            
            # for i in range(self.envs.num_envs):
            #     each_action_list = low_actions[i]
            #     new_images_i = {k:[] for k in keys}
            #     for act in each_action_list:
            #         act = act.strip(".")
            #         outputs = self.envs.step([{'action':{'action': actions_dict[act], 'action_args':{}}}])
            #         observations, _, dones, infos = [list(x) for x in zip(*outputs)]
            #         if dones[i]:
            #             break
            #         for k in keys:
            #             new_images_i[k].append(observations[i][k][None,...])
            #     distance = self.envs.call_at(i, "current_dist_to_goal")
            ...
            low_loss += self.policy.net(**target_inputs)
        ...
        if train_ml is not None:
            ...
            low_loss = low_loss/total_actions
            self.loss += il_loss
            self.loss += low_loss
    ...
    def _eval_checkpoint(
        ...
        evaluated_path = "/root/mount/cevln/Visual-Language-Navigation/data/logs/eval_results/hamt_seg_low_6751/stats_ep_ckpt_0_val_unseen_r0_w1.json"
        with open(evaluated_path) as f_eval:
            evaluated_data = json.load(f_eval)
        evaluated_id = list(evaluated_data.keys())
        evaluated_id = [int(i) for i in evaluated_id]
        ...
        while len(state_episodes) < episodes_to_eval:
            ...
            # if envs.current_episodes()[0].episode_id in evaluated_id:
            #     continue
            ...
            for i in range(envs.num_envs):
                open_list = []
                instance_id_to_name = envs.call_at(i, "get_instance_id", {})
                for key, value in instance_id_to_name.items():
                    if value in open_vocab:
                        open_list.append(key)
            ...
            instr_max_len = self.config.IL.max_text_len # r2r 80, rxr 200
            instr_pad_id = 1 if self.config.MODEL.task_type == 'rxr' else 0
            #obs_land = self.landmark_search([(ob["instruction"]['trajectory_id'], ob["instruction"]['text']) for ob in observations])
            observations = extract_instruction_tokens(observations, self.config.TASK_CONFIG.TASK.INSTRUCTION_SENSOR_UUID,
                                                    max_length=instr_max_len, pad_id=instr_pad_id)
            batch = batch_obs(observations, self.device)
            ...
            # for img_key in keys:
            #     img = Image.fromarray(batch[img_key][0].cpu().detach().numpy(),'RGB')
            #     img.save('/root/mount/cevln/waypoint-predictor/images/raw_images/test%s.png'%str(img_key))
            #     img.show()
            ...
            for stepk in range(self.max_len):
                ...
                target_inputs = {
                'mode': 'low_action_infer',
                'txt_masks': txt_masks,
                'states': t_outputs[1]
                }
              
               
                low_actions = self.policy.net(**target_inputs)
                for i in range(envs.num_envs):
                    each_action_list = low_actions[i].split(" ")[3:]
                    new_images_i = {k:[] for k in keys}
                    for act in each_action_list:
                        act = act.strip(".")
                        outputs = envs.step([{'action':{'action': actions_dict[act], 'action_args':{}}}])
                        observations, _, dones, infos = [list(x) for x in zip(*outputs)]
                        if dones[i]:
                            break
                        for k in keys:
