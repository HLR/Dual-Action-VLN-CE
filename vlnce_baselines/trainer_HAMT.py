...
with open("/root/mount/cevln/landmarks/val_unseen.json") as f_land:
...
class RLTrainer(BaseVLNCETrainer):
    ...
    def _initialize_policy(
        ...
        # self.waypoint_predictor = BinaryDistPredictor_TRM(device=self.device)
        # self.waypoint_predictor.load_state_dict(
        #     torch.load('/root/mount/dcvln-joslin/data/pretrained/wp_pred/waypoint_predictor', map_location = torch.device('cpu'))['predictor']['state_dict']
        # )
        ...
        while len(state_episodes) < episodes_to_eval:
            ...
            for i in range(envs.num_envs):
                ...
                for s_k in semantic_keys:
                    ...
                    # obs_tensor = torch.zeros(1,224,224,3, dtype=torch.uint8)
                    # obs_tensor[0] = torch.from_numpy(observations[i]['rgb'])*torch.from_numpy(semantic_obs).unsqueeze(-1)
                    # semantic_obs = torch.from_numpy(np.array(semantic_obs, dtype=np.float32))
                    # show_rgb_img = Image.fromarray(obs_tensor[0].cpu().detach().numpy(),'RGB')
                    # show_rgb_img.save('rgbimgs.png')
                    # show_rgb_img.show()
            ...
            for stepk in range(self.max_len):
                ...
                for i in range(envs.num_envs):
                    ...
                    for s_k in semantic_keys:
                        ...
                        for open_v in open_list:
                            semantic_obs = np.where(semantic_obs==open_v, -1, semantic_obs)
                        semantic_obs = np.where(semantic_obs==-1, 0, 1)
                        observations[i][s_k] = semantic_obs
