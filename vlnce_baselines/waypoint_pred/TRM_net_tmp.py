...
def TRM_predict(mode, args, predictor, rgb_feats, depth_feats):
    ...
    vis_logits, depth_logits = predictor(rgb_feats, depth_feats)
    # entry-wise probabilities
    vis_probs = torch.sigmoid(vis_logits)

    if mode == 'train':
        return vis_logits, depth_logits
    elif mode == 'eval':
        return vis_probs, vis_logits


class BinaryDistPredictor_TRM(nn.Module):
    def __init__(self, args=None, hidden_dim=768, n_classes=12, device=None):
        ...
        self.projection1 = nn.Linear(512, 768)
    ...
    def forward(self, rgb_feats, depth_feats):
        ...
        #     bsi, self.num_imgs, -1)
        rgb_x = self.projection1(rgb_feats.float())
        rgb_x = rgb_x.reshape(bsi, self.num_imgs, -1)
